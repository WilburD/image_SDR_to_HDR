function fused = TooDark(I, mu, k, a, b) % camera a, b
%%
...
if  ~exist( 'k', 'var' ) || isempty(k) %k为曝光率，没有k则：先计算k，再计算曝光图J   
    ...
    isBad = t_our > 0.7;  %是过亮光照的像素点
    %sum(sum(isBad~=0));
    if sum(sum(isBad~=0)) == 0
        tmpt = t_our(:);
        tmpt = sort(tmpt,'descend');
        idx = uint64(size(tmpt)/8);
        idx = idx(1) + 1;
        threshold = tmpt( idx );
        isBad = t_our > threshold;
    end
    J = maxEntropyEnhance(I, isBad,0.01,0.99);  %求k和曝光图
else                                               %有k则：直接根据k计算曝光图
    J = applyK(I, k, a, b); %k
